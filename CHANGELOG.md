...
### Added — `fluentcrm/fluentcrm-custom-optin-forms`

The load-bearing correction: **`FluentCrmApi('contacts')->createOrUpdate()` does not send a double opt-in email.** It stores the contact and syncs the supplied relations, and that is all — the canonical core flow is two deliberate calls, `createOrUpdate([... 'status' => 'pending'], false, false)` followed by `sendDoubleOptinEmail()` *only when the resulting status is actually `pending`*. Integrations that assume one call does both ship forms that silently never confirm anyone, or — worse — reach for `$forceUpdate = true` to make the form "work" and turn an anonymous endpoint into a way to resubscribe a suppressed address without proving mailbox control. FluentCRM already owns the tokenized confirmation URL, the public confirmation request, the `pending` → `subscribed` transition, funnel resumption and the system note; the skill says plainly not to build a second token, table, or confirmation route around that lifecycle. Double opt-in and the confirmation hook are **Free-core** features, so a signup integration must not require Pro for the base flow.

An **existing-contact status matrix** covers all seven statuses, because "create a contact" is the easy half and "this address is already here" is where policy lives: `pending` gets safe field updates and a resend request (core suppresses successful resends for 150 seconds), `subscribed` is never demoted or re-mailed by default, `transactional` may move to `pending` on explicit marketing consent, `unsubscribed` requires a deliberate re-consent flow through `updateStatus('pending')` and never a direct jump to subscribed, and `bounced` / `complained` / `spammed` stay suppressed in a normal public form pending an administrative remediation policy.

Two authorization traps get named directly. **List/tag IDs from request data are not an authorization boundary** — the sanitizer accepts names and slugs and may *create* missing definitions, and a numeric ID being well-formed proves nothing about whether the visitor may select it; resolve a server-owned choice map and verify the definitions exist. And **a REST `permission_callback` returning `true` can be correct** for an intentionally anonymous newsletter route while leaving the handler completely unsafe: a WordPress REST nonce does not authenticate a logged-out visitor, and core's 150-second guard is per existing contact after a successful send — it does nothing against an attacker submitting many *new* addresses. The endpoint section therefore requires strict field allowlists, per-IP and per-normalized-email throttling *before* contact creation, bounded request size and per-request work, honeypot / minimum-fill-time / CAPTCHA where abuse risk warrants, and generic responses that do not let the form enumerate who is already in the CRM.

Also verified in 3.1.13 source: the double opt-in sender selects list-specific settings from the contact's **latest list pivot row** (`created_at DESC, id DESC`), and re-attaching an already-attached list is a no-op that does *not* become latest again — so multi-list contacts can silently get a different template than a fresh signup, and the skill's advice is one explicit DOI-driving list per form with a global-settings fallback when precedence is ambiguous. Attach hooks fire immediately and do **not** wait for confirmation, so a "verified subscriber" tag belongs on `fluent_crm/subscriber_confirmed_via_double_optin` — scoped with a plugin-owned subscriber-meta marker, or the callback also processes confirmations started by unrelated forms. `sendDoubleOptinEmail() === false` means "not sent now" (wrong status, resend window, or invalid DOI configuration), not a failed contact write; a `true` return means the send path ran, not that delivery or human intent was proven. The skill closes on the honest limit: a clicked link demonstrates link access, and automated mail scanners follow GET links — DOI is not strong identity verification for high-risk account actions. A ten-point audit checklist and `references/implementation-contract.md` carry the endpoint contract and status handling; ships an `agents/openai.yaml`.
...
Domain README gained the new skill row and an intro line for public signup forms; root README domain row and skill counter 255 → 256; `skills-index.json` regenerated.

## 2026-08-22 (elementor: classic `Widget_Base` widget development, 2 new skills)

The domain covered Dynamic Tags, large-dataset pickers, markup experiments and deprecations, but had **no skill for building an actual widget** — the most common reason someone writes an Elementor addon. Two skills close that gap, grounded against **Elementor Free 4.2.3 / Pro 4.2.2** on WordPress 7.1 with verified source paths (`includes/base/widget-base.php`, `controls-stack.php`, `element-base.php`, `includes/managers/widgets.php`, `controls.php`, `elements.php`, `includes/controls/`, `includes/elements/repeater.php`, `assets/js/frontend.js`, plus `includes/widgets/heading.php` and `icon-list.php` as native models).

Both use **V3 to name the classic editor/widget architecture, not an installed 3.x version** — the `Widget_Base` / `Controls_Stack` model is still available and still what core widgets use on 4.2.3. Atomic Widgets / Editor V4 is deliberately out of scope while its prop, control and style contracts keep moving, and the skills state the boundary rather than implying it: an identity test table maps `Widget_Base` / `Controls_Manager` / `register_controls()` to in-scope and `Modules\AtomicWidgets` / `Atomic_Widget_Base` / prop types to out-of-scope, the two models may coexist only behind separate classes and registration paths, and a V3 control array must never be translated into an Atomic schema by guesswork. The Atomic boundary itself was verified in `modules/atomic-widgets/`.

### Added — `elementor/elementor-v3-widget-development`

The addon-lifecycle half: fail-safe companion bootstrap (`Requires Plugins`, `did_action( 'elementor/loaded' )`, `ELEMENTOR_VERSION` and PHP gates, and the rule that a `Widget_Base` subclass file must not be included before Elementor exists or a missing plugin fatals before the notice can run), registration through `elementor/widgets/register` with `$widgets_manager->register( new Widget() )` (`widgets_registered` deprecated since 3.5.0), and **`get_name()` as persisted identity** — it becomes `widgetType` in saved Elementor JSON, drives CSS classes and the frontend-ready hook, so renaming it breaks existing content and a collision silently replaces another plugin's registered object.

Rendering is framed as PHP-canonical: `get_settings_for_display()` versus raw `get_settings()`, re-validating enumerations at output time because saved JSON / imports / filters / dynamic tags bypass the editor's option list, escaping at the final context, `add_render_attribute()` / `print_render_attribute_string()` / `add_link_attributes()`, `add_inline_editing_attributes()` and `get_repeater_setting_key()`. `content_template()` is optional Backbone parity — never the place for authorization or sensitive lookups — and `render_plain_content()` gets its own decision (search/SEO/feeds, or an empty string for functionality that must not survive deactivation). Assets register once on a WP enqueue hook and reach the page through `get_style_depends()` / `get_script_depends()`; frontend JS initializes from `frontend/element_ready/{name}.default` idempotently and `$scope`-bound, because editor rerenders fire it repeatedly and DOM-ready alone misses them. Output caching gets the strictest rule: `is_dynamic_content()` defaults to `true`, and returning `false` requires proving cross-user, cross-request output stability — Elementor detecting configured dynamic tags does not prove arbitrary PHP is static. `references/widget-contract-and-example.md` carries the full bootstrap, a complete widget, the frontend handler pattern and a five-part release test matrix.

### Added — `elementor/elementor-v3-widget-controls`

The editor-contract half, built on **built-in controls only** (custom control classes stay out — a separate lifecycle, asset and compatibility problem). Starts from output and data shape rather than panel appearance, because `URL`, `MEDIA`, `ICONS`, `SLIDER`, `DIMENSIONS`, `GALLERY` and `REPEATER` each return a structured array, not a string — a value-shape table pairs every control with what `render()` actually receives, and dedicated renderers (`Icons_Manager`, attachment APIs, link attributes) handle the ones that need them.

The load-bearing rule is that **control definitions are not a security boundary**: a `SELECT` option list does not constrain an imported, filtered or database-set value, a UI range does not prove the saved number is in range, a dynamic tag can replace the value at display time, and a condition-hidden control still exists in raw document data while its display value may be `null`. Conditions are panel UX, not authorization; `frontend_available` is exposure, not secure transport. Also covers wrapper-scoped `selectors` with `{{VALUE}}` / `{{SIZE}}` / `{{UNIT}}` / `{{CURRENT_ITEM}}`, `selectors_dictionary` and allowlisted `prefix_class` instead of interpolating editor text into declarations, simple `condition` versus compound `conditions` (and Elementor's documented non-support for an inner repeater field depending on an outer control), group controls without recreating their internal IDs, and repeaters keyed by `_id` rather than array order with bounded per-row work. Includes a genuine 4.x testing trap: with control optimization active, style controls live in a separate `style_controls` stack that `get_controls()` merges only when `Performance::is_use_style_controls()` holds for the context — so assert registration with `get_controls( 'known-id' )`, never by inventorying `array_keys( get_controls() )` from a frontend/CLI context. Catalog, value shapes, selector tokens, group controls, the repeater pattern and an output-policy matrix live in `references/built-in-controls-and-patterns.md`.

Both skills ship an `agents/openai.yaml` and cross-link each other plus `elementor-experiments-and-markup` (icon rendering and the wrapper markup that optimized-markup mode removes) and `elementor-dynamic-tag-ajax-select` (large datasets, Pro-detect with a free-safe manual-ID fallback). The domain README intro was rewritten to lead with widget development and to state the V3-naming and Atomic-exclusion decision up front; root README domain row updated, skill counter 253 → 255, `skills-index.json` regenerated.

## 2026-08-22 (wordpress: post-password content gating)

### Added — `wordpress/wp-password-protected-content`

WordPress's built-in shared-password gate, grounded against **WordPress 7.1** source (`wp-includes/post-template.php`, `wp-login.php`, `class-wp.php`, `class-wp-query.php`, `comment.php`, the posts/comments REST controllers, and the post-meta / post-data block bindings). The skill exists because `post_password` is routinely mistaken for authentication: the value is stored **plaintext** in `wp_posts.post_password`, the body is not encrypted, and the browser cookie is a **site-scoped** proof — it is not keyed by post ID, so posts sharing a password unlock together and only one distinct password works per browser at a time. There is no per-reader identity, revocation list, attempt counter, or audit trail, and logged-in users do not bypass the frontend check. A decision table sends per-user grants, hidden post existence, private files, and high-value data to authenticated authorization instead.

The load-bearing rule is that **core protects the ordinary content/excerpt path, not your data access**. A built-in output matrix records what 7.1 actually covers (singular content/excerpt, post classes, core post-data/post-meta bindings, comments, feeds/enclosures, logged-out `WP_Query` search, the core posts REST response) against what does not — `get_post()`, `get_post_field()`, raw `$post->post_content`, `get_post_meta()` and custom tables, `$wpdb`, applying `the_content` to a raw string, featured images and public uploads URLs, custom REST/GraphQL routes, and search indexers. Same for listings: `has_password` / `post_password` are `WP_Query` **selection** filters, not access checks, and the anonymous search exclusion does not extend to archives, custom SQL, REST collections, third-party indexes, or logged-in users.

Extension guidance is contract-preserving rather than rip-and-replace: patch `the_password_form` with `WP_HTML_Tag_Processor` so the action, field names, `redirect_to`, unique label/input IDs, error announcement and translations survive — a full-form rewrite also silently drops 7.1's `.wp-block-button` / `.wp-block-button__link` wrapper and its enqueued block style. Bypasses are framed as authorization policy (`current_user_can( 'edit_post', … )` is defensible for editorial preview; `read_post` is not a membership check, and the filter reaches comments, blocks, feeds and REST preparation alike). REST/headless covers the single-item `password` parameter, empty `content.rendered` with `protected: true`, `rest_post_incorrect_password` / 403, `context=edit`, and the fact that a password in a query string lands in browser history, proxy/CDN logs, analytics and traces. Caching gets its own contract — core's no-cache headers only help once the request reaches WordPress, so an edge cache must bypass protected URLs and `wp-postpass_` cookies, and purging never fixes a design that mixes locked and unlocked variants.

Ships a HIGH/MEDIUM/LOW/INFO severity guide with one deliberate false-positive guard: the core form's missing nonce is **not** CSRF on its own, since the handler only changes the visitor's own post-password cookie. Also scoped out explicitly — login/reset/Application Passwords, membership and paywall systems, whole-site protection, and legacy code that merely reuses the `post_password` column for internal values (Action Scheduler claim IDs, old WooCommerce order keys). Contracts for the runtime sequence, the six extension hooks (`post_password_required`, `the_password_form`, `the_password_form_incorrect_password`, `post_password_expires`, `protected_title_format`, `login_form_postpass`), REST behavior, queries and caching live in `references/runtime-and-extension-contracts.md`; the skill ships an `agents/openai.yaml`. Domain README row, skill counter (252 → 253), and `skills-index.json` updated.
...
- **`lw-plugins/lw-firewall-rate-limit-worker`** — Make a plugin compatible with the MU-plugin worker: the fixed check order (kill switch → `enabled` → server/localhost skip → `ip_whitelist` → `ip_blacklist` → geo → auto-ban → 404 flood → bot UA → endpoint detection → rate limit), the detected request types (`cron`/`xmlrpc`/`login`/`rest`/`filter`), narrow `filter_params` substring rules with per-prefix limits, endpoint-local self-protection via `RateLimiter::is_allowed_key()` + `AutoBanner::record_violation()` + `lw_firewall_resolve_storage()` (with namespaced keys, never the worker's `login_*`/`rest_*`/`filter_*` keys), the `IpDetector::get_ip()` Cloudflare-range trust model, whitelist/blacklist CIDR handling, worker lifecycle (install/remove/version-mismatch bail/`LW_FIREWALL_DISABLE_WORKER`), and response codes. `plugin: lw-firewall`, `plugin-version-tested: "1.3.2"`, `php-min: "8.1"`.
- **`lw-plugins/lw-firewall-registration-guard`** — Wire a custom registration form into LW Firewall's spam protection. The plugin auto-protects only the core `register_form` / `registration_errors` (when `users_can_register`); custom/AJAX/REST/Woo/CRM/LMS signups opt in. Covers the token (`lw_fw_reg_token`) + honeypot (`lw_fw_url`) fields, the preferred server-rendered path (`RegisterGuard::render_fields()` + `RegisterGuard::validate( WP_Error )`), headless token issue (`RegisterToken::issue()`), manual validation (`RegisterToken::verify( $token, $min_fill_time, $max_age, $storage )`), single-use replay protection, generic error messaging (never leak which check failed), and reject-count auto-ban shared with the worker store — plus the reminder that the token is anti-spam, not a WordPress nonce. `plugin: lw-firewall`, `plugin-version-tested: "1.3.2"`, `php-min: "8.1"`.
- **`lw-plugins/lw-firewall-management-abilities`** — Automate or audit firewall management across its three surfaces: WP-CLI (`wp lw-firewall status|config|config-items|ip|worker`), LW Site Manager abilities (`lw-firewall/get-options`, `get-log`, `list-blocked`, `block-ip`, `unblock-ip`, all `can_manage_options`), and admin import/export. Covers the single-option model (`Options::get_all()` / `save()` merge-with-defaults, list-typed keys), constants winning over options, why abilities touch only the manual `ip_blacklist` (auto-bans stored separately), the capped `lw_firewall_log`, MU-worker status/reinstall, and safe config migration. `plugin: lw-firewall`, `plugin-version-tested: "1.3.2"`, `php-min: "8.1"`.

### New skill — `polylang/polylang-compatibility-audit` (Polylang 3.8.5 / Pro 3.8.5 / Woo 2.2.2)

The audit / verdict companion to the six Polylang implementation skills — the Polylang analogue of `wpml-compatibility-audit`. Where the others tell you *how* to make one surface compatible, this one tells you *what to inspect* across a whole plugin/theme and *how to classify* what breaks.

- **`polylang/polylang-compatibility-audit`** — Audit whether a plugin or classic theme is compatible with Polylang, Polylang Pro, and Polylang for WooCommerce end to end, and return a verdict (Compatible / Partially compatible / Not Polylang-compatible / Not testable) plus a severity-tagged findings-and-fix report. Walks each surface in order: stack detection (`POLYLANG_VERSION` / `POLYLANG_PRO` / `PLLWC_Data_Store`), static gettext vs admin-entered dynamic strings (`pll_register_string()` + `pll__()` / `PLL_Translate_Option`), stored object IDs & URLs needing `pll_get_post` / `pll_get_term` / `pll_home_url` (with the `0`-on-miss trap and the "don't concatenate `/$lang/`" rule), CPT/taxonomy opt-in and its timing, model-based creation/import/sync (`pll_insert_post` / `pll_save_post_translations` — never write the private language/translation taxonomies), queries & custom tables (Polylang does not language-filter custom SQL — needs a `lang` column or a documented snapshot), REST/AJAX/headless `lang` propagation, Pro-only assumptions guarded behind `POLYLANG_PRO` (translated/shared slugs, sync, ACF), WooCommerce data-store paths, and the highest-risk async output (cron, webhooks, PDFs, emails, exports, CLI) that must choose an explicit language. Includes verdicts, dynamic-string and stored-ID checklists, a common-findings severity table, and a report format; defers to the six implementation skills and `wp-i18n-audit` for the actual fixes. `plugin: polylang + polylang-pro + polylang-wc`, `plugin-version-tested: "Polylang 3.8.5 + Polylang Pro 3.8.5 + Polylang for WooCommerce 2.2.2"`, `php-min: "7.4"`.

### Updated — `lw-plugins/lw-elallas-integration` (Elállás for WooCommerce 1.0.11 → 1.0.12)

Re-grounded against Elállás for WooCommerce 1.0.12 with several new extension points and refreshed source line references (SKILL.md + reference.md).

- New `elallas_resolve_order_number` filter (`(int $order_id, string $number)`, return `0` to fall through) — the order the customer types is the **display** order number, resolved via this filter → WooCommerce Sequential Order Numbers Pro/free helpers → native ID fallback; compatibility code must never `absint()` the public form value. Cross-references the new `wc-sequential-order-numbers-pro` skill.
- `elallas_case_status_changed` is now **4-arg** (`…, string $message`) — the optional customer-visible admin note (empty for REST / LW Site Manager transitions); register `accepted_args` as `4` to read it.
- Documented product/category/tag withdrawal exceptions (`_lw_elallas_excluded` / `_lw_elallas_exclusion_reason`, seven reason keys) which flag case items as `eligibility_flag = 'excepted'` for manual review rather than hard-blocking.
- Rewrote the multilingual section around **runtime** WPML/Polylang translation of admin-entered strings and `Multilingual::object_id()` page resolution, with the intentionally narrow `wpml-config.xml` scope (do not double-register the option strings as WPML `<admin-texts>`).
- Added the PII-scrubbed WooCommerce logging surface (source `elallas-for-woo`) and the `elallas/get-audit-log` Site Manager ability; updated all verified line numbers to 1.0.12.

### Repo / docs

- `lw-plugins/README.md`: intro now lists LW Firewall; three new firewall skill rows; the `lw-elallas-integration` row updated to v1.0.12 coverage.
- `polylang/README.md`: skills table grew a `polylang-compatibility-audit` row (positioned as the audit companion below the implementation skills).
- Root `README.md`: the `lw-plugins/` domain blurb now reflects Elállás and LW Firewall.
- Each new skill ships an `agents/openai.yaml` interface descriptor (same pattern as the other `lw-plugins` and `polylang` skills).
- `skills-index.json` regenerated (`skill_count` 173 → 177; `lw-plugins` 8 → 11; `polylang` 6 → 7; `domain_count` unchanged at 19 — both are existing domains, so no allowlist changes were needed).

## 2026-07-07

### New domain — `learndash/` (LearnDash LMS + LearnDash WooCommerce)

A new top-level domain for making **your own** plugin, theme, import, CRM/checkout bridge, or headless client cooperate with [LearnDash LMS](https://www.learndash.com/) (`sfwd-lms`) and the LearnDash WooCommerce bridge (`learndash-woocommerce`) — integration and audit skills, not a re-implementation. The mental model that anchors the batch: **course access is not one usermeta key.** In LearnDash 5.1.6.1 a user can reach a course through direct enrollment, group enrollment, open courses, sample steps, admin auto-enroll, and free/paynow settings, all gated by product start/end windows and expiration — so authorization goes through `sfwd_lms_has_access()`, and grant/revoke goes through the public helpers (`ld_update_course_access()`, `ld_update_group_access()`, `ld_update_course_group_access()`) that also write access activity, historical `enrolled_at` timestamps, clear transients, and fire hooks — never a hand-written `course_{id}_access_from` / `learndash_group_users_{id}` meta. The running theme: **use the access API to decide access, use the helpers to change it, and respect the WooCommerce bridge's own model** — product/variation `_related_course` / `_related_group` relations, configurable grant/revoke statuses, partial refunds, subscription switches, HPOS-safe order CRUD, the silent enrollment queue, and the per-user access counter that stops one order from revoking access another still grants. Every skill is grounded in local source references against LearnDash LMS 5.1.6.1 and LearnDash WooCommerce 2.0.2.

### New skills (LearnDash LMS 5.1.6.1 / LearnDash WooCommerce 2.0.2)

- **`learndash/learndash-course-access`** — Build or audit course access and enrollment. The canonical gate is `sfwd_lms_has_access( $post_id, $user_id )` and its full decision chain (no related course, missing course, admin auto-enroll / sample steps / open courses, free/paynow, direct `course_{id}_access_from` meta, group access, the `Product` start/end window, `ld_course_access_expired()`, and the `sfwd_lms_has_access` filter) — so checking only the direct meta misses group and open-course access. Covers granting/revoking through `ld_update_course_access()` and the modern `LearnDash\Core\Models\Product` model (and everything the helper does beyond writing meta: access activity, `learndash_course_{id}_enrolled_at`, the enrollment hook, transient deletion, and why revoke intentionally keeps the historical timestamp), `ld_course_access_from_update()` for historical access dates, the read APIs (`ld_course_access_from`, `ld_course_access_expires_on`, `learndash_user_get_enrolled_courses`, `learndash_get_users_for_course`) with the forced-`$bypass_transient` and open-course query caveats, and the course-steps outline APIs (creating lesson/topic/quiz posts is not course structure). `plugin: sfwd-lms`, `plugin-version-tested: "5.1.6.1"`, `php-min: "7.4"`.
- **`learndash/learndash-group-access`** — Build or audit group membership, group-course access, group leaders, and hierarchical groups. Frames the model as three separate relationships (user → group, group → course, leader → group) and maps each to the correct helper (`ld_update_group_access`, `ld_update_course_group_access`, `ld_update_leader_group_access`, plus the `learndash_set_group_enrolled_courses` / `learndash_set_course_groups` diff helpers) and the meta each lives in (user meta vs course postmeta). Covers the read APIs (`learndash_is_user_in_group`, `learndash_get_users_group_ids`, `learndash_group_enrolled_courses`, `learndash_get_course_groups`, `learndash_user_group_enrolled_to_course`), hierarchical-group expansion that a direct `learndash_group_users_{id}` scan misses, the 5.x transient changes to several group helpers, and why group access can make `sfwd_lms_has_access()` true with no direct course meta at all. `plugin: sfwd-lms`, `plugin-version-tested: "5.1.6.1"`, `php-min: "7.4"`.
- **`learndash/learndash-woocommerce-access`** — Build or audit the LearnDash WooCommerce bridge that grants/revokes course and group access from Woo products, orders, refunds, subscriptions, switches, and order-item changes. Corrects "on completed order call `ld_update_course_access()`": the bridge tracks product/variation `_related_course` / `_related_group` relations (arrays, stored on the variation when sold through variations, not the parent), reads configurable grant/revoke statuses from `learndash_woocommerce_enrollment_status_settings` (defaults include `wc-processing` / `wc-completed` for orders and `wc-active` / `wc-pending-cancel` / `wc-on-hold` for subscriptions), handles partial refunds via `woocommerce_order_refunded`, subscription lifecycle and switches, order-item mutations and customer changes, declares HPOS compatibility (use Woo order CRUD, not postmeta SQL), can enqueue large orders into `learndash_woocommerce_silent_course_enrollment_queue`, and — critically — maintains the per-user `_learndash_woocommerce_enrolled_courses_access_counter` so one purchase/refund does not revoke access another order still grants. `plugin: learndash-woocommerce`, `plugin-version-tested: "LearnDash WooCommerce 2.0.2 + LearnDash LMS 5.1.6.1"`, `php-min: "7.4"`.
- **`learndash/learndash-rest-api`** — Build or audit REST / headless integrations. LearnDash 5.1.6.1 exposes two surfaces: classic content/relationship CRUD under `ldlms/v2` (courses, lessons/topics/quizzes, groups, users, course steps, enrollments, group memberships, progress) and a newer `learndash/v1` manifest/OpenAPI discovery surface whose experimental endpoints require the `Learndash-Experimental-Rest-Api: allow` header. Covers enablement (`LEARNDASH_REST_API_ENABLED` + the settings section, the `learndash_rest_api_enabled` filter), the default v2 route bases and why not to hard-code them (settings can override), the nested ID-keyed `/steps` request body (not `[123, 456]` arrays), the enrollment/relation endpoints and their permission model (open-course rejection via `learndash_rest_rejected_course_open`, admin-only relation writes, limited group-leader access), a headless integration checklist, and extending safely with a separate namespaced controller and explicit permission callbacks. Cross-references `wp-rest-api` for generic REST security/schema patterns. `plugin: sfwd-lms`, `plugin-version-tested: "5.1.6.1"`, `php-min: "7.4"`.

### Repo / docs

- New `learndash/README.md` with the 4-skill table and a domain intro (integration-not-reimplementation framing; the access-is-not-one-meta-key / grant-through-the-helpers / respect-the-Woo-bridge theme; verified against LearnDash LMS 5.1.6.1 and LearnDash WooCommerce 2.0.2).
- Root `README.md` domain table grew a `learndash/` row.
- `learndash` added to the three domain allowlists in lockstep — `.github/scripts/validate-skill.js`, `.github/scripts/build-skill-pr.js`, and the `.github/ISSUE_TEMPLATE/new-skill.yml` domain dropdown.
- `learndash-woocommerce-access` `plugin-version-tested` normalized from `"2.0.2; sfwd-lms 5.1.6.1"` to the repo's `" + "`-joined dual-plugin convention (`"LearnDash WooCommerce 2.0.2 + LearnDash LMS 5.1.6.1"`) so it reads consistently with the TranslatePress / Polylang / FluentCRM multi-plugin skills.
